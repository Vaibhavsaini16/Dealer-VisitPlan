...
export const EditStaffForm: React.FC<EditStaffFormProps> = ({ staff, setView, updateStaff }) => {
    const [name, setName] = useState(staff.name);
    const [email, setEmail] = useState(staff.email);
    const [phone, setPhone] = useState(staff.phone);
    const [role, setRole] = useState<Staff['role']>(staff.role);
    ...
    return (
        <div className="max-w-2xl mx-auto animate-fade-in">
            <button onClick={() => setView('VIEW_STAFF')} className="flex items-center gap-2 text-slate-600 hover:text-slate-900 font-medium mb-6 transition-colors">
                <ArrowLeftIcon className="w-5 h-5" />
                Back to Staff List
            </button>
            <div className="bg-white p-8 rounded-lg shadow-lg border border-slate-200">
                <h2 className="text-2xl font-bold text-slate-800 mb-1">Edit Staff Member</h2>
                <p className="text-slate-500 mb-6">Update the details for {staff.name}.</p>
